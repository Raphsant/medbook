...
const { data } = await useAsyncData<DataRecord[]>(
  async () => {
    ...
    return dates.map((date) => {
      ...
      const appointmentsForDate = props.apts.filter((appointment) => {
        // Convert both dates to a comparable format, e.g., "YYYY-MM-DD"
      ...
      return {
        date,
        appointments: appointmentsForDate, // Replace the amount with appointments
      };
    });
  },
  {
...
const y = (d: DataRecord) => d.appointments.length;

const total = computed(() =>
  data.value.reduce(
    (acc: number, record: DataRecord) => acc + record.appointments.length,
    0,
  ),
);
...
const template = (d: DataRecord) =>
  `${formatDate(d.date)}: ${d.appointments.length}`;
</script>
