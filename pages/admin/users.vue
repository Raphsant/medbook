...
const {pending, data, error} = await useFetch('https://postgresapp-e83cc2ceb04b.herokuapp.com/api/user/getAll', {
  method: 'GET',
...
<template>
    ...
    <UDashboardPanel grow>
      ...
      <UDashboardToolbar>
        <template #left>
          ...
          <UButton label="Eliminar Paciente" icon="i-heroicons-trash" variant="outline" color="red" @click="isDeleteUserModalOpen = true"/>
        </template>
      ...
      <UTable
          ...
          :progress="{ color: 'primary', animation: 'carousel' }" :columns="defaultColumns" :rows="data"
          :loading="pending"></UTable>
    </UDashboardPanel>
  </UDashboardPage>
  <UDashboardModal
  ...
  >
    <template #footer>
      ...
      <UButton color="white" label="Cancelar" @click="isDeleteUserModalOpen = false"/>
    </template>
  </UDashboardModal>
